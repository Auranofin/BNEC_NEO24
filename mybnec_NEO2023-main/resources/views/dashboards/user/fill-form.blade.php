<x-app title="Participant Registration | Fill Form">
                                ...
                                <div class="card-body">
                                    <div class="card-body d-flex justify-content-between">
                                        ...
                                        <div>
                                            ...
                                            @elseif($ticket->competitionName == "Essaywriting")
                                    ...
                                    <div class="card-body">
                                        ...
                                        <ul class="nav nav-tabs" style="border-block-end:1px solid #38AAC4 !important;">
                                    ...
                                    @elseif($ticket->competitionName == 'Essaywriting')
                                    <div class="card-body">
                                        <div class="mb-4" id="essaywritingTeam">
                                            <label class="form-label">Team Name</label>
                                            <input type="text" class="form-control" name="essaywriting_team_name[{{ $i }}]" placeholder="Essay Writing Team Name">
                                        </div>
                                        <div style="display: flex; flex-flow: row; gap: 10px;" id="typeHandler">
                                            <label>
                                                <input type="radio" name="essaywriting_participant_type[{{$i}}]" value="individual" x-model="participantType" required>
                                                I wish to participate individually
                                            </label>

                                            <label>
                                                <input type="radio" name="essaywriting_participant_type[{{$i}}]" value="team" x-model="participantType" required>
                                                I wish to participate as a team
                                            </label>
                                        </div>

                                        <ul class="nav nav-tabs" style="border-block-end:1px solid #38AAC4 !important;">
                                            <li class="nav-item col-6">
                                                <a class="nav-link text-center active" data-toggle="tab" href="#Essaywriting-{{$i}}-speaker1">Speaker 1</a>
                                            </li>
                                            <li class="nav-item col-6">
                                                <a class="nav-link text-center" data-toggle="tab" href="#Essaywriting-{{$i}}-speaker2">Speaker 2</a>
                                            </li>
                                        </ul>
                                    </div>

                                    <div class="tab-content">
                                        <div class="tab-pane fade show active" id="Essaywriting-{{$i}}-speaker1">
                                            <div class="mb-3">
                                                <x-user.participant-details-input prefix="Essaywriting-{{$i}}-1" />
                                            </div>
                                        </div>
                                        <div class="tab-pane fade" id="Essaywriting-{{$i}}-speaker2">
                                            <div class="mb-3">
                                                <x-user.participant-details-input prefix="Essaywriting-{{$i}}-2" />
                                            </div>
                                        </div>
                                    </div>
                                    @else
    ...
    <script>
        ...
        $('#typeHandler input[type="radio"]').on('change', function() {
    // Check which radio button is selected
            var selectedValue = $(this).val();

            if (selectedValue === 'individual') {
        // If 'individual' is selected, disable the team name input
                $('#essaywritingTeam .form-control').prop('disabled', true).val('');
                $('#individualForm').show();
                $('#teamForm').hide(); // Clear the value if you want
            } else if (selectedValue === 'team') {
        // If 'team' is selected, enable the team name input
                $('#essaywritingTeam .form-control').prop('disabled', false);
                $('#teamForm').show();
                $('#individualForm').hide();
            }
        });
    ...
    <script>
        ...
        document.addEventListener('alpine:init', (e) => {
            Alpine.data('emailValidation', () => ({
                ...
                individualParticipation: false, // Track whether participating as an individual
                init() {
            // Automatically disable Speaker 2 tab when individualParticipation is true
            this.$watch('individualParticipation', (value) => {
                if (value) {
                    // Hide the second speaker's form
                    $('#Essaywriting-{{$i}}-speaker2').addClass('d-none');
                } else {
                    // Show the second speaker's form
                    $('#Essaywriting-{{$i}}-speaker2').removeClass('d-none');
                }
            });
        }
            }))
        ...
        document.addEventListener('alpine:init', () => {
            Alpine.data('participantTypeSelection', () => ({
                participantType: 'individual',
                get isIndividual() {
                    return this.participantType === 'individual';
                },
                handleParticipantTypeChange() {
                    const essaywritingTeamInput = document.getElementById('essaywritingTeam');
                    essaywritingTeamInput.style.display = this.isIndividual ? 'none' : 'block';
                }
...
<style>
    ...
    a.nav-link.active {
        ...
        background: #38AAC4 !important;
        border-block-end: 1px solid #38AAC4 !important;
    }
