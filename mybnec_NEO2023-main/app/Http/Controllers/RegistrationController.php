...
use App\Models\EssayWritingTeams;
...
class RegistrationController extends Controller
{
     ...
     */
     ...
     private function createEssayWritingParticipant($request, $registrationDetail, $competitionIndex, $participantIndex)
     {
    // Get province and district data
    $province = Province::find($request->address_provinceID['Essaywriting-' . $competitionIndex . '-' . $participantIndex]);
    $district = District::find($request->address_disctrictID['Essaywriting-' . $competitionIndex . '-' . $participantIndex]);

    // Get institution (Binusian or other university)
    if (empty($request->binusian_nim['Essaywriting-' . $competitionIndex . '-' . $participantIndex]) &&
        empty($request->binusian_region['Essaywriting-' . $competitionIndex . '-' . $participantIndex])) {
        $institution = $request->university_name['Essaywriting-' . $competitionIndex . '-' . $participantIndex];
    } else {
        $institution = $request->binusian_name['Essaywriting-' . $competitionIndex . '-' . $participantIndex];
    }

    // Create the participant
    $participant = Participant::create([
        'name' => ucwords($request->participant_name['Essaywriting-' . $competitionIndex . '-' . $participantIndex]),
        'registration_detail_id' => $registrationDetail->id,
        'gender' => $request->participant_gender['Essaywriting-' . $competitionIndex . '-' . $participantIndex],
        'email' => $request->participant_email['Essaywriting-' . $competitionIndex . '-' . $participantIndex],
        'phone_number' => $request->participant_phone['Essaywriting-' . $competitionIndex . '-' . $participantIndex],
        'university_year' => $request->grade['Essaywriting-' . $competitionIndex . '-' . $participantIndex],
        'province' => $province->name,
        'district' => $district->name,
        'address' => $request->address_fullname['Essaywriting-' . $competitionIndex . '-' . $participantIndex],
        'institution' => $institution,
        'allergy' => $request->allergy['Essaywriting-' . $competitionIndex . '-' . $participantIndex],
        'level' => 1,
    ]);

    // If participant is a Binusian, store extra Binusian info
    if ($participant->institution == 'Bina Nusantara University') {
        Binusian::create([
            'participant_id' => $participant->id,
            'nim' => $request->binusian_nim['Essaywriting-' . $competitionIndex . '-' . $participantIndex],
            'region' => $request->binusian_region['Essaywriting-' . $competitionIndex . '-' . $participantIndex],
            'faculty' => $request->binusian_faculty['Essaywriting-' . $competitionIndex . '-' . $participantIndex],
            'major' => $request->binusian_major['Essaywriting-' . $competitionIndex . '-' . $participantIndex],
        ]);
    }
}
            ...
            foreach ($competitions as $competition) {
                ...
                if ($quantity > 0) {
                    ...
                    for ($i = 0; $i < $quantity; $i++) {
                        ...
                        } else if ($competition->name == 'Essaywriting') {
                            $participantType = $request->essaywriting_participant_type[$i + 1];  // Get participant type (individual or team)
                            
                            // Create an EssayWritingTeam entry
                            EssayWritingTeams::create([
                                'registration_detail_id' => $registrationDetail->id,
                                'participant_type' => $participantType,  // Store participant type (individual or team)
                                'team_name' => $participantType == 'team' ? $request->essaywriting_team_name[$i + 1] : NULL,  // Only store team name if it's a team
                            ]);
                            
                            // If individual participant, create only one participant
                            if ($participantType == 'individual') {
                                $this->createEssayWritingParticipant($request, $registrationDetail, $i + 1, 1);
                            } 
                            // If it's a team, create two participants
                            else if ($participantType == 'team') {
                                for ($participantIndex = 1; $participantIndex <= 2; $participantIndex++) {
                                    $this->createEssayWritingParticipant($request, $registrationDetail, $i + 1, $participantIndex);
                                }
                            }
                        }else{
